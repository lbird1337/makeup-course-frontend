...
const PaymentModal = ({ tariff, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center" onClick={onClose}>
      <div className="bg-white text-black p-8 rounded-none w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
        <h2 className="font-heading text-2xl font-bold mb-2">Подтверждение заказа</h2>
        <p className="font-body mb-6 border-b border-gray-300 pb-4">Тариф: <span className="font-bold">{tariff.description}</span></p>
