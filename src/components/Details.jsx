    ...
    <section className="py-20 px-4 sm:px-6 md:px-8">
      <div className="container mx-auto">
        <div className="flex items-center mb-12">
          <h2 className="font-heading text-fluid-2xl font-black text-brand-accent uppercase tracking-widest whitespace-nowrap">Важные детали</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {detailsData.map((section, sectionIndex) => (
            ...
            <div key={sectionIndex} className="border border-brand-text p-8 h-full">
              <h3 className="h3 mb-6">{section.title}</h3>
              <div className="space-y-6">
                {section.items.map((item, itemIndex) => (
                  <div key={itemIndex} className="flex items-start gap-4">
                    <p className="font-heading font-black text-xl text-brand-accent pt-1">
                      {String(itemIndex + 1).padStart(2, '0')}.
                    </p>
                    <p className="font-body text-brand-text flex-1">
                      {item}
                    </p>
                  </div>
                ))}
