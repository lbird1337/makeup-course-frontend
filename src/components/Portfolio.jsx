...
const Portfolio = () => {
  ...
  const ImageItem = ({ index, src }) => (
    <div className="aspect-square cursor-pointer overflow-hidden" onClick={() => openLightbox(index)}>
      <img src={src} alt={`Portfolio ${index + 1}`} className="w-full h-full object-cover transition-transform duration-300 hover:scale-110" />
    </div>
  );
      ...
      <section className="py-20 px-4 sm:px-6 md:px-8">
        <div className="container mx-auto">
          <div className="flex items-center mb-12">
            <h2 className="font-heading text-4xl font-black text-brand-accent uppercase tracking-widest">Портфолио</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            {previewImages.map((image, index) => (
              <ImageItem key={index} index={index} src={image.src} />
            ))}
            
            {overlayImage && (
              <div className="aspect-square relative cursor-pointer overflow-hidden" onClick={() => openLightbox(numVisibleImages - 1)}>
                <img src={overlayImage.src} alt={`Portfolio ${numVisibleImages}`} className="w-full h-full object-cover"/>
